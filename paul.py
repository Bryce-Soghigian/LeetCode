...
def find_codes(graph, node, code, codes, cache):
    if len(code) == 9:
        codes.append(code)
        return
    if (node, code) in cache:
        return
    cache.add((node, code))
    for neighbor in graph[node]:
        if neighbor not in code:
            find_codes(graph, neighbor, code + neighbor, codes, cache)
graph = {}
graph['a'] = ['b', 'd', 'e']
graph['b'] = ['a', 'c', 'e']
graph['c'] = ['b', 'e', 'f']
...
cache = set()
for node in graph:
    find_codes(graph, node, node, codes, cache)
